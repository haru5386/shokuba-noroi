...
import { curseLevels } from './data/curses';
import { Howl } from 'howler';
import type { FC } from 'react';
import type { Item } from './index.type';
const characterImgs: string[] = Array.from({ length: 8 }, (_, i) => require(`./assets/character-${i + 1}.png`));
const itemImgs: string[] = Array.from({ length: 5 }, (_, i) => require(`./assets/item-${i + 1}.png`));
const defaultRoles = [
  '主管', 'PM', '資深前端', '嘴砲王',
...
function getCurse(role: string, rage: number): string {
  const yellowRole = `<span class="text-yellow-400 font-bold">${role}</span>`;
  if (rage <= 10) return curseLevels.low[Math.floor(Math.random() * curseLevels.low.length)].replace('{職位}', yellowRole);
  if (rage <= 20) return curseLevels.mid[Math.floor(Math.random() * curseLevels.mid.length)].replace('{職位}', yellowRole);
  return curseLevels.high[Math.floor(Math.random() * curseLevels.high.length)].replace('{職位}', yellowRole);
}
...
const App: FC = () => {
  ...
  // 發洩模式
  const handleRageClick = ({ x, y }: { x: number; y: number }) => {
    if (rage >= 30) return;
    punchSound.play();
    setRage(rage + 1);
    setItems([...items, {
    ...
    timerRef.current = setTimeout(() => {
      setShowCurse(true);
      setCurseText(getCurse(role, rage + 1));
      bombSound.play();
    }, 1500);
  };
